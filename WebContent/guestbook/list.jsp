...
<%
	...
	list = dao.getSelectAll_02(search_option, search_data, startRecord, lastRecord);
	int count = list.size();
...
%>	
		<tr>
			<td><%=jj %></td>
			<td>
			<a href="../main/main.jsp?menu_gubun=guestbook_view&no=<%=dto.getNo()%>">
...
<%			jj--;//전체 레코드의 갯수를 나타내기 때문에 하나씩 빼준다.
} %>	
</table>
...
<%	if(totalRecord > 0){%>	
	<tr>
		<td style="padding-top: 20px; text-align: center;">
			<a href="#" onclick="Gopage('guestbook_list', '1', '<%=search_option%>', '<%=search_data%>');">[첫페이지]</a> &nbsp;
		
			<%if(startPage > blockSize){ //시작페이지가 10개의 블럭 사이즈보다 크다고 하면 작동%>
			<a href="#" onclick="Gopage('guestbook_list', '<%=startPage - blockSize%>', '<%=search_option%>', '<%=search_data%>');">
			[이전10개]</a> &nbsp;
			<%}else{ %>
				[이전10개]&nbsp;
			<%} %>
			...
			<%for(int i = startPage; i <= lastPage; i++){%>
				<%if(pageNumber == i){%>
					<b>[<%=i %>]</b> &nbsp;
				<%}else{ %>
					<a href="#" onclick="Gopage('guestbook_list', '<%=i%>', '<%=search_option%>', '<%=search_data%>');">
					<%=i %>
					</a>&nbsp;
				<%} %>
			<%}%>
			...
			<a href="#" onclick="Gopage('guestbook_list', '<%=startPage + blockSize%>', '<%=search_option%>', '<%=search_data%>');">[다음 10개]</a> &nbsp;
			<%}else{ %>
			[다음 10개] &nbsp;
			<%} %>
			<a href="#" onclick="Gopage('guestbook_list', '<%=totalPage%>', '<%=search_option%>', '<%=search_data%>');">[끝 페이지]</a>
		</td>
	</tr>
<%} %>	
		<!-- 페이징 종료 -->
...
<script>
	function search() {
		if(confirm('검색하시겠습니까?')){
			...
			document.searchForm.action = "../main/main.jsp?menu_gubun=guestbook_search";
	...
	function Gopage(value1, value2, value3, value4){		
		var arg = "";
		arg = arg + "/jsp_mvc1/main/main.jsp";
		arg = arg + "?menu_gubun="+value1;
		arg = arg + "&pageNumber="+value2;
		arg = arg + "&search_option="+value3;
		arg = arg + "&search_data="+value4;
		location.href = arg;		
	}	
