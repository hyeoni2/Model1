...
<%
	...
	int totalRecord = dao.getTotalRecord();//전체 레코드 갯수
	...
	ArrayList<ProductDTO> list = dao.getSelectAll(search_option, search_data);
	int count = list.size();
%>
<style>
	a{
...
%>
	<tr>
		...
		<td>
		<a href="../main/main.jsp?menu_gubun=product_view&product_code=<%=dto.getProduct_code()%>">
			<b style="color: 4A0084"><%=dto.getProduct_code() %></b>
		</a>
		</td>
	...
	<tr>
		<td>
			...
			<% if(search_option.equals("name")){ %>
					...
					<option value="name" selected>상품 이름</option>
					<option value="maker">상품 브랜드</option>
					<option value="name_maker">이름+브랜드</option>
			
			<%}else if(search_option.equals("maker")){ %>
					<option value="" >-- 선택 --</option>
					<option value="name">상품 이름</option>
					<option value="maker" selected>상품 브랜드</option>
					<option value="name_maker">이름+브랜드</option>
			...
			<%}else if(search_option.equals("name_maker")){ %>
					...
					<option value="name" >상품 이름</option>
					<option value="maker">상품 브랜드</option>
					<option value="name_maker" selected>이름+브랜드</option>
			<%}else if(!(search_option.equals("name") || search_option.equals("maker") || search_option.equals("name_maker"))){ %> 
...
<%	if(totalRecord > 0){%>	
	<tr>
		<td style="padding-top: 20px; text-align: center;">
			<a href="../main/main.jsp?menu_gubun=product_list&pageNumber=1&search_option=<%=search_option%>&search_data=<%=search_data%>">[첫페이지]</a> &nbsp;
			...
			<%for(int i = startPage; i <= lastPage; i++){%>
				<%if(pageNumber == i){%>
					<b>[<%=i %>]</b> &nbsp;
				<%}else{ %>
					<a href="../main/main.jsp?menu_gubun=product_list&pageNumber=<%=i %>"><%=i %></a>&nbsp;
				<%} %>
			<%}%>
			...
			[다음 10개] &nbsp;
			<%} %>
			<a href="../main/main.jsp?menu_gubun=product_list&pageNumber=<%=totalPage%>&search_option=<%=search_option%>&search_data=<%=search_data%>">[끝 페이지]</a>
		</td>
	</tr>
<%} %>	
		<!-- 페이징 종료 -->
...
<script>
	function search() {
		if(confirm('검색하시겠습니까?')){
			...
			searchForm.action = '../main/main.jsp?menu_gubun=product_search';
			searchForm.submit();
