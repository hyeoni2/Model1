...
<%
	...
	ArrayList<SungjukDTO> list = dao.getSelectAll_02(search_option, search_data, startRecord, lastRecord);
	int count = list.size();
%>
...
<%if(search_data == null || search_data.trim().equals("")){ %>	
	전체 회원목록 조회
<%}else{ %>
	검색어 <b style="color: #4A0084;"><%=search_data %></b>로 총 [<b style="color: blue;"><%=count %></b>]건 검색완료
<%} %>			
</div>
	<table id="table">
	<tr>
...
%>
	<tr>
		<td><%=jj%></td>
		<td>
		<a href="../main/main.jsp?menu_gubun=sungjuk_view&no=<%=dto.getNo()%>">
			<b style="color: #4A0084;"><%=dto.getName()%></b>
		</a></td>
		<td><%=dto.getSihum_name()%></td>
		<td><%=dto.getKor()%></td>
		<td><%=dto.getEng()%></td>
		<td><%=dto.getMat()%></td>
		<td><%=dto.getSci()%></td>
	...
	<tr>
		<td>
			<form name="searchForm">
				<select name="search_option" style="width: 110px;">
			
			<% if(search_option.equals("name")){ %>
					<option value="" >-- 선택 --</option>
					<option value="name" selected>이름</option>
					<option value="sihum_name">시험 이름</option>
					<option value="name_sihum_name">이름+시험이름</option>
			
			<%}else if(search_option.equals("sihum_name")){ %>
					<option value="" >-- 선택 --</option>
					<option value="name">이름</option>
					<option value="sihum_name" selected>시험 이름</option>
					<option value="name_sihum_name">이름+시험이름</option>
			
			<%}else if(search_option.equals("name_sihum_name")){ %>
					<option value="" >-- 선택 --</option>
					<option value="name" >이름</option>
					<option value="sihum_name">시험 이름</option>
					<option value="name_sihum_name" selected>이름+시험이름</option>
			<%}else if(!(search_option.equals("name") || search_option.equals("sihum_name") || search_option.equals("name_sihum_name"))){ %> 
...
<%	if(totalRecord > 0){%>	
	<tr>
		<td style="padding-top: 20px; text-align: center;">
			<a href="../main/main.jsp?menu_gubun=sungjuk_list&pageNumber=1&search_option=<%=search_option%>&search_data=<%=search_data%>">[첫페이지]</a> &nbsp;
			...
			<%for(int i = startPage; i <= lastPage; i++){%>
				<%if(pageNumber == i){%>
					<b>[<%=i %>]</b> &nbsp;
				<%}else{ %>
					<a href="../main/main.jsp?menu_gubun=sungjuk_list&pageNumber=<%=i %>"><%=i %></a>&nbsp;
				<%} %>
			<%}%>
			...
			[다음 10개] &nbsp;
			<%} %>
			<a href="../main/main.jsp?menu_gubun=sungjuk_list&pageNumber=<%=totalPage%>&search_option=<%=search_option%>&search_data=<%=search_data%>">[끝 페이지]</a>
		</td>
	</tr>
<%} %>	
		<!-- 페이징 종료 -->
...
<script>
	function search() {
		if(confirm('검색하시겠습니까?')){
			...
			document.searchForm.action = "../main/main.jsp?menu_gubun=sungjuk_search";
