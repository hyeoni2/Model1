...
<%
	...
	//전체저장용 리스트 생성
	ArrayList<EmployeeDTO> list = dao.selectAllwith_dep_searchPaging(search_option, search_data, startRecord, lastRecord);
	int count = list.size();
...
%>
	...
	<tr>
		<td><%=jj %></td>	
		<td>
			<a href="../main/main.jsp?menu_gubun=employee_view&employee_id=<%=dto.getEmployee_id()%>">
				<b style="color: #4A0084;"><%=dto.getEmployee_id() %></b>
			</a>	
		</td>	
			...
			<form name="searchForm">
					<select name="search_option">
						<%if(search_option.equals("name")){%>
							...
							<option value="name_department_id" >이름+부서번호</option>
						...
						<%}else if(search_option.equals("name_department_id")){%>	
							...
							<option value="name" >이름</option>
							<option value="department_id" >부서번호</option>
							<option value="name_department_id" selected >이름+부서번호</option>
						<%}else{%>
...
<%	if(totalRecord > 0){%>	
	<tr>
		<td style="padding-top: 20px; text-align: center;">
			<a href="#" onclick="Gopage('employee_list', '1', '<%=search_option%>', '<%=search_data%>');">[첫페이지]</a> &nbsp;
		
			<%if(startPage > blockSize){ //시작페이지가 10개의 블럭 사이즈보다 크다고 하면 작동%>
			<a href="#" onclick="Gopage('employee_list', '<%=startPage - blockSize%>', '<%=search_option%>', '<%=search_data%>');">
			[이전10개]</a> &nbsp;
			<%}else{ %>
				[이전10개]&nbsp;
			<%} %>
			...
			<%for(int i = startPage; i <= lastPage; i++){%>
				<%if(pageNumber == i){%>
					<b>[<%=i %>]</b> &nbsp;
				<%}else{ %>
					<a href="#" onclick="Gopage('employee_list', '<%=i%>', '<%=search_option%>', '<%=search_data%>');">
					<%=i %>
					</a>&nbsp;
				<%} %>
			<%}%>
			...
			<a href="#" onclick="Gopage('employee_list', '<%=startPage + blockSize%>', '<%=search_option%>', '<%=search_data%>');">[다음 10개]</a> &nbsp;
			<%}else{ %>
			[다음 10개] &nbsp;
			<%} %>
			<a href="#" onclick="Gopage('employee_list', '<%=totalPage%>', '<%=search_option%>', '<%=search_data%>');">[끝 페이지]</a>
		</td>
	</tr>
<%} %>	
		<!-- 페이징 종료 -->
